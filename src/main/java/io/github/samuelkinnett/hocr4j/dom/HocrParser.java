...
import io.github.samuelkinnett.hocr4j.Page;
import io.github.samuelkinnett.hocr4j.utils.ListWrappingQueue;
...
public final class HocrParser {
    ...
    static int elementLength(String hocr, int offset) {
        ...
        if (hocr.charAt(offset) == '<') {
            ...
            if (openedAt < 0) {
                ...
                return nonnegative(openedAt - offset);
            }
        }
    }
    ...
    static List<HocrElement> createAst(Queue<String> tokens) {
        ...
        while (!tokens.isEmpty()) {
            ...
            if (t.startsWith("<") && t.endsWith(">")) {
                ...
                if (t.endsWith("/>")) {
                    result.add(new HocrTag(t, new ArrayList()));
                } else {
    ...
    public static List<Page> parse(List<HocrElement> elements, int startingPageNumber) {
        ...
        List<HocrElement> bodyContents = new HocrTag("<>", elements).findTag("body").getElements();
