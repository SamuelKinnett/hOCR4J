...
public final class MatchWords {
    ...
    static private class Parser {
        ...
        ArrayList<ListW2SM> parse() {
            return new ArrayList<ListW2SM>(parse(0, 0, new ArrayList()));
        }

        @SuppressWarnings("unchecked")
        Set<ListW2SM> parse(int wordOffset, int stringOffset, List<Word> prefix) {
            ...
            if (headWord.getText().length() <= 1) {
                buffer.addAll(parse(wordOffset + 1, stringOffset, prefix));
            }
            if (stringOffset < strings.size()) {
                ...
                if (TextUtils.fuzzyEqual(headString, prefixPlusHeadWord)) {
                    Set<ListW2SM> tails =
 parse(wordOffset + 1, stringOffset + 1, new ArrayList());
                    final WordToStringMatch head = new WordToStringMatch(CollectionUtils.pushBack(prefix, headWord), headString);
                    buffer.addAll(CollectionUtils.<ListW2SM, ListW2SM>setMap(tails, ListW2SM.cons_(head)));
                }
                if (TextUtils.fuzzyPrefix(headString, prefixPlusHeadWord)) {
                    buffer.addAll(parse(wordOffset + 1, stringOffset, CollectionUtils.pushBack(prefix, headWord)));
                }
            }
