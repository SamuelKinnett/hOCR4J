    ...
    public Word(@Nonnull HocrElement e) {
        ...
        while (true) {
            if (e instanceof HocrTag) {
                ...
                if (tag.getName().equals("span") && ("ocrx_word".equals(tag.getTagClass()) || "ocr_word".equals(tag.getTagClass()))) {
                    _bounds = Bounds.fromHocrTitleValue(tag.getTitle());
                    _confidence = getConfidenceFromTitle(tag.getTitle());
                } else if (tag.getName().equals("strong") || tag.getName().equals("b")) {
                    _isBold = true;
                } else if (tag.getName().equals("em") || tag.getName().equals("i")) {
                    _isItalic = true;
                } else {
                    throw new IllegalArgumentException(e.mkString());
                }
                if (tag.getElements().isEmpty()) {
                    break;
                } else {
                    e = tag.getElements().get(0);
                }
            } else {
                break;
            }
